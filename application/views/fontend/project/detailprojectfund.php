...
<section style="width:100%;background:white;padding:2%;">
	...
	<div class="col-sm-9">
		...
		<span style="font-size:16;color:#253747;padding:0;">
			...
			<br>
			&nbsp; &nbsp; &nbsp;	<a href="<?=base_url()?>assets/img/project/documentproject/<?=$detailproject[0]['project_pdfpath']?>"><?=($detailproject[0]['project_pdfpath']!='' ? $detailproject[0]['project_pdfpath'] : '-' )?></a>
			<br>
		</span>
							...
							<?php
								if($detailproject[0]['member_member_id']==$member_logindata['member_id']){
									if($member_logindata['MemberRole']=="Moderator"){
										echo "<h6 class='comment-name by-author'><a href='#''>".$member_logindata['member_name']."</a></h6><span style='color:white;background:#03658c;padding:1px 7px;margin:5px 4px;'><b>ผู้ดูแลระบบ</b></span>";
									}
									else{
										echo "<h6 class='comment-name by-author'><a href='#''>".$member_logindata['member_name']."</a></h6>";
									}
								}else{
									if($member_logindata['MemberRole']=="Moderator"){
										echo "<h6 class='comment-name'><a href='#''>".$member_logindata['member_name']."</a></h6><span style='color:white;background:#03658c;padding:1px 7px;margin:5px 4px;'><b>ผู้ดูแลระบบ</b></span>";
									}else{
										if(count($checkStaff)!=0){
											foreach ($checkStaff as $key => $staffValue) {
												if($staffValue['member_id'] == $member_logindata['member_id']){
													echo "<h6 class='comment-name'><a href='#''>".$member_logindata['member_name']."</a></h6><span style='color:white;background:#03658c;padding:1px 7px;margin:5px 4px;'><b>สต๊าฟโครงการ</b></span>";
												}else{
													echo "<h6 class='comment-name'><a href='#''>".$member_logindata['member_name']."</a></h6>";
												}
											}
										}else{
											echo "<h6 class='comment-name'><a href='#''>".$member_logindata['member_name']."</a></h6>";
						...
						<div class='comment-main-level'>
							...
							<div class='comment-box'>
								...
								if($detailproject[0]['member_member_id']==$value['member_id']){
									echo "<h6 class='comment-name by-author'><a href='#''>".$value['member_name']."</a></h6>";
							  	}else{
									echo "<h6 class='comment-name' ><a href='#''>".$value['member_name']."</a></h6>";
							  	}
							...
							echo "<li>
									...
									<div class='comment-box'>
										...
										if($detailproject[0]['member_member_id']==$subcomment['member_id']){
											echo "<h6 class='comment-name by-author'><a href='#''>".$subcomment['member_name']."</a></h6>";
									  	}else{
											echo "<h6 class='comment-name'><a href='#''>".$subcomment['member_name']."</a></h6>";
									  	}
